...
if($_SERVER["REQUEST_METHOD"] == "POST"){
    ...
    $sql = "INSERT INTO editeur (NomEditeur, AdresseEditeur) VALUES (?, ?)";
    $t = $cnx->prepare($sql);
    $t->execute([$nom, $adresse]);
    header("Location: editeurs.php");
    exit();
}
...
<body class="min-h-screen bg-[#0a0f1e] text-slate-100 flex">
    ...
    <aside class="w-64 min-h-screen glass border-r border-white/5 flex flex-col py-8 px-4 fixed top-0 left-0">
        ...
        <nav class="flex flex-col gap-1">
            ...
            <a href="editeurs.php"  class="px-4 py-3 rounded-xl text-sm font-semibold bg-yellow-500/10 text-yellow-400 border border-yellow-500/20">🏢 Editeurs</a>
    ...
    <main class="ml-64 flex-1 p-8">
        ...
        <div class="glass rounded-2xl p-6 mb-8 flex justify-between items-center">
            <div>
                ...
                <h2 class="fancy text-3xl font-bold text-white mt-1">Les <span class="text-yellow-400">Éditeurs</span></h2>
            </div>
        </div>

        <!-- FORM -->
        <div class="glass rounded-2xl p-6 mb-8 border border-white/5">
            <h3 class="text-lg font-bold text-white mb-4">➕ Ajouter un Éditeur</h3>
            <form method="POST" class="flex flex-col sm:flex-row gap-4">
                <input type="text" name="nom" required placeholder="Nom de l'éditeur"
        ...
        <div class="glass rounded-2xl p-6 border border-white/5">
            <h3 class="text-lg font-bold text-white mb-4">📋 Liste des Éditeurs</h3>
                ...
                <table class="w-full text-left">
                    ...
                    <tbody class="divide-y divide-white/5">
                        ...
                        <tr class="hover:bg-white/[0.02] transition-colors">
                            <td class="p-4 text-sm text-slate-400"><?= $e['NumEditeur'] ?></td>
                            <td class="p-4 text-sm text-white font-semibold"><?= htmlspecialchars($e['NomEditeur']) ?></td>
                            <td class="p-4 text-sm text-slate-300"><?= htmlspecialchars($e['AdresseEditeur']) ?></td>
                        </tr>
                        <?php endforeach; ?>
                        <?php if(count($editeurs) == 0): ?>
                        <tr>
                            <td colspan="3" class="p-8 text-center text-slate-500 text-sm">Aucun éditeur pour le moment.</td>
                        </tr>
                        <?php endif; ?>
                    </tbody>
                </table>
